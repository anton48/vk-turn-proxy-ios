...
ключ должен совпадать с ключом заданным на сервере FreeTurn через параметр -obf-key. Создать его можно вручную (32 байта hex, 64 символа) или сгенерировать на сервере через однократный запуск с ключом -gen-obf-key
