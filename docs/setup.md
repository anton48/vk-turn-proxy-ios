...
- URL VK звонка (или адрес/порт TURN relay) или несколько URL
...
В поле VK Call Link необходимо добавить один или несколько URL звонка из VK. 
...
Переключатель VK Account Auth включает работу в неанонимном режиме. 
...
- приватный ключ клиента (на стороне WireGuard сервера в разделе [Peer] этого клиента должен быть парный ему публичный ключ)
- публичный ключ WireGuard сервера
- опционально preshared key (должен быть равным PresharedKey на стороне WireGuard сервера в разделе [Peer] этого клиента, в общем случае оставляем пустым)
- IP адрес и маску (должны соответствовать AllowedIPs на стороне WireGuard сервера в разделе [Peer] этого клиента)
...
В этом режиме появляются дополнительные поля для ввода пароля WDTT и Device ID, а раздел настроек WireGuard исчезает (так как WDTT сервер присылает их клиенту после установления соединения в автоматическом режиме):
...
- Device ID: идентификатор устройства, который передается серверу WDTT в запросе GETCONF (скриншот выше сделан до появления этого поля). Сервер привязывает к нему выданный клиенту WireGuard peer и IP адрес, поэтому менять его не нужно — при изменении сервер выдаст новый адрес в туннеле. Поле заполняется автоматически при выборе режима; менять его имеет смысл только чтобы получить на сервере отдельную запись для этого сервера из списка. В ссылки (vkturnproxy:// и wdtt://) Device ID не входит: это идентификатор устройства, а не настройка сервера, поэтому при импорте ссылки создается новый. В файл полного бэкапа он сохраняется. 
